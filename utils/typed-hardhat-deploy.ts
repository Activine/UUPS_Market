...
interface TypedDeployments<CustomNames extends Record<string, keyof Factories>>
  extends DeploymentsExtension {
  deploy<N extends keyof Factories>(
    name: N,
...
type _Factories0 = {
  [key in keyof _Typechain as key extends `${infer N}__factory`
    ? N
    : never]: InstanceType<_Typechain[key]>;
};
type Factories = Pick<
  _Factories0,
  {
...
interface TypedDeployOptionsWithContract<N extends keyof Factories>
  extends TypedDeployOptions<N> {
  contract: N;
}
